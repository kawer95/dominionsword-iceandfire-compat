...
要求：Forge 1.20.1、当前 Dominion Sword 开发版、Ice And Fire CE 1.2.7（Forge）、Uranus 2.x。
...
```powershell
.\gradlew.bat build `
  -Pdominionsword_jar="..\DominionSword-1.20.1\build\libs\dominionsword-当前版本.jar" `
...
- 不对 Dominion Sword 声明版本范围；附属始终随本体当前 API 重新构建和验证。
...
2. 准备当前 Dominion Sword JAR、`IceAndFireCE-1.2.7-1.20.1-forge.jar`、`uranus-2.2.6-bugfix.2-1.20.1-forge.jar`。
...
普通攻击默认进入悬停龙息；`orbit_breath`（盘旋持续 AOE）与 `strafe_breath`（掠袭单次 AOE，冷却 900 tick）通过技能栏释放，并随当前 Dominion Sword 技能 API 构建。
